...
export default {
    ...
    data() {
        return {
            tilesArray: [
                ...
                { value: 0,    position: [3, 3], moveable: true }
            ],
    ...
    methods: {
        ...
        getBlankTile: function() {
            let blankTile = this.tilesArray.filter( tile => tile.value == 0);
        ...
        moveTiles: function(tileValue) {
            //let clickedTileValue = this.tilesArray.filter(tile => tile.value == tileValue) ;
            // .filter returns a new array with values that match
            this.activeTile = tileValue;
            
            let activeTile = this.activeTile;
            let blankTile = this.blankTile.value;

            console.log(`ClickedTile: ${activeTile}, BlankTile: ${blankTile}`);
